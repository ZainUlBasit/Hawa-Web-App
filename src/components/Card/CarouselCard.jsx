...
const CarouselCard = ({ Icon, title, img, desc, i }) => {
  ...
  return (
    ...
    >
      {isActive ? (
        <div className="overlay absolute top-0 left-0 w-full h-full bg-[rgba(0,0,0,.6)] z-1 fade-in"></div>
      ) : (
        <div className="overlay absolute top-0 left-0 w-full h-full bg-[rgba(0,0,0,.4)] z-1"></div>
      )}
